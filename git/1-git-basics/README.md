...
SHA-1 방식은 분명 다른 데이터를 가지고 같은 해쉬 값을 만들 위험이 있지만, 확률이 아주 희박하고. 주변의 다른 체크섬(커밋)을 참고하면 바뀐 파일과 데이터가 무엇이 었는지 확정할 수 있다. 
...
저자는 코드 작업을 수행한 원작자, 커미터는 마지막으로 이 작업을 저장소에 적용한 사람을 의히한다.
...
push를 한 커밋이라면, ammend를 사용할 경우 `git push -f`를 사용해야하므로 새로운 커밋을 만드는게 더 나은것 같습니다. (Force는 데이터 손실이 있을 수 있기 때문)
